...
\end{verbatim}
    ...
    used in a pair on some memory address
    
    for efficient implementation:
    \begin{itemize}
      \item false negative can happen during store-conditional (stxr instruction): a chunk of memory tracked usually 64 bytes / 1 kB
      \item 1 memory address pre core can be tracked at a time
      \item clrex (stop tracking writes to memory) $\implies$ subsequent SC will fail
    \end{itemize}
