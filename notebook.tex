...
\end{verbatim}
    ...
    \begin{itemize}
      ...
      \item use of \verb%unsafe%:
        ...
        \item eg: \verb%std::mem::MaybeUninit<T>% (unsafe version of \verb%Option<T>%)
        \end{itemize}
      \item can use \verb%UsafeCell% for interior mutability when sharing across multiple threads
      \item use type checking and static analysis to avoid errors: move/consume to avoid unwanted reuse of resources
      \item use of \verb%Arc<T>% for sharing and resource dropping: \verb%RX% drop and \verb%TX% drop $\implies$ \verb%Arc<T>% drop $\implies$ \verb%T% drop
      \item allocation optimization: user borrowed channel instead of creating an \verb%Arc% and managing it: use of lifetimes and mutable borrow for compile time checks
